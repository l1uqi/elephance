...
| [SkillClaw: Let Skills Evolve Collectively with Agentic Evolver](https://arxiv.org/abs/2604.08377) | 多用户、跨时间的轨迹和失败模式可以让 skill/rule 更快集体演化。 | Elephance 当前仍保持本地优先；路线图会把 shared rule repository、可选遥测、推广门槛和团队规则同步作为后续扩展，而不是默认后台行为。 |
